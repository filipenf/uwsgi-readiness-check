...
def shutdown():
    ...
    parser.add_argument('--shutdown-progress-file',
    ...
    if graceful_shutdown(
            ...
            args.shutdown_progress_file, args.max_wait, args.uwsgi_master_fifo
    ) < 1:
        exit(-1)
...
def is_ready():
    parser = argparse.ArgumentParser()
    parser.add_argument('--verbose', default=False, action='store_true')
    parser.add_argument('--stats-socket')
    parser.add_argument('--queue-threshold', help='uwsgi queue threshold (a value between 0 and 1). Default: 0.5',
                        default=0.5)
    parser.add_argument('--shutdown-progress-file',
                        default=DEFAULT_SHUTDOWN_PROGRESS_FILE,
                        help='If this file exists, readiness checks will fail')
